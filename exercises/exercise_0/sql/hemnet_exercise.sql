...
SELECT * FROM main.hemnet LIMIT 100;
...
SELECT COUNT(DISTINCT commune) AS number_communes
FROM main.hemnet;
...
WITH milli AS (
	SELECT
		...
		COUNT(CASE WHEN final_price > 10000000 THEN 1 END) AS number_milli,
	FROM main.hemnet
)
SELECT (number_milli / number_rows) * 100.0 AS percentage
FROM milli;
